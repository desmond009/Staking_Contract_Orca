...
export const NetworkWarning = ({ account, isCorrectNetwork, onSwitchNetwork }) => {
  ...
  return (
    <div className="mt-4 rounded-xl border border-error/40 bg-error/10 p-3 text-xs text-error sm:mt-6 sm:rounded-2xl sm:p-4 sm:text-sm">
      <p className="font-medium">Wrong network detected.</p>
      <p className="mt-1 text-[10px] text-error/80 sm:text-xs">
        Please switch to {TARGET_NETWORK_NAME} to interact with the staking contract.
      </p>
      <button
        type="button"
        onClick={onSwitchNetwork}
        className="mt-2.5 w-full rounded-lg bg-error/20 px-3 py-1.5 text-xs font-medium text-error transition hover:bg-error/30 sm:mt-3 sm:rounded-xl sm:px-4 sm:py-2 sm:text-sm"
      >
...
export const NetworkConnectionWarning = ({ hasErrors = false }) => {
  ...
  return (
    <div className="mt-4 rounded-xl border border-yellow-500/40 bg-yellow-500/10 p-3 text-xs text-yellow-200 sm:mt-6 sm:rounded-2xl sm:p-4 sm:text-sm">
      <p className="font-medium">Network connection issues detected</p>
      <p className="mt-1 text-[10px] text-yellow-200/80 sm:text-xs">
